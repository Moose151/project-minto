...
const UI = {
  ...
  topbar(){
    ...
    const phase = G.phase==='regular' ? `Round ${G.round+1} of ${G.fixtures.length}` : G.phase==='finals' ? (G.finals.week===1?'Semi Finals':'Grand Final') : 'Off-Season';
    el.innerHTML = `
      ...
      <div class="top-stat"><span class="lbl">Season</span><span class="val">${G.year} · ${esc(phase)}</span></div>
